...
@app.route('/api/groups/<group_id>/project-info', methods=['GET'])
def get_project_info_api(group_id):
    """Get group's project information"""
    ...
    try:
        # Get project_title from group
        response = supabase_client.table('groups').select('project_title').eq('id', group_id).execute()
        ...
        # Return empty project info if group not found
        return jsonify({
            'title': '',
            'description': '',
            'objectives': '',
        ...
        logger.error(f"Error getting project info for group {group_id}: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500


@app.route('/api/groups/<group_id>/project-info', methods=['PUT'])
def update_project_info_api(group_id):
    ...
    try:
        ...
        # Store only the title in project_title column (truncate to 255 chars if needed)
        title = data.get('title', '')[:255]

        response = supabase_client.table('groups').update({
            'project_title': title
        }).eq('id', group_id).execute()

        # The update response may not contain data, but we can check if it succeeded
        ...
        logger.error(f"Error updating project info for group {group_id}: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500
